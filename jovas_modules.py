...
class NativeDBConnection:
    ...
    def ask(self, args):
        ...
        # Table detection — 4 strategies
        table = None
        for n in self.tables:                        # exact match
            if n in q: table = n; break
        if not table:
            for n in self.tables:                    # singular of table
                if n.rstrip("s") in q: table = n; break
        if not table:
            for w in words:                          # query word ~= table
                for n in self.tables:
                    if w == n or w == n+"s" or w.rstrip("s") == n:
                        table = n; break
                if table: break
        if not table:
            table = next(iter(self.tables), None)    # fallback: first table
        if not table:
        ...
        if "inactive" in q:
            where["active"] = False
        elif re.search(r"\bactive\b", q):
            where["active"] = True

        # Role — matches singular AND plural (admin/admins, user/users)
        # but skip if the only matching word is the table name itself
        for role in ["admin", "moderator", "guest", "user"]:
            if re.search(r"\b" + role + r"s?\b", q):
                # Don't set role filter if the only occurrence is the table name
                # e.g. "find all users" — "users" is the table, not a role filter
                match_words = re.findall(r"\b" + role + r"s?\b", q)
                non_table   = [w for w in match_words if w != table and w != table + "s"]
                if non_table:
                    where["role"] = role; break

        # Status
        for status in ["pending", "delivered", "cancelled", "paid", "unpaid"]:
            if f"status {status}" in q or f"{status} status" in q or status in q:
                where["status"] = status; break
        ...
        if m:
            order = (m.group(2), "DESC" if "desc" in (m.group(3) or "") else "ASC")
...
class RealHttpModule:
    ...
    def _request(self, method, url, body=None, headers=None):
        ...
        try:
            with urllib.request.urlopen(req, timeout=self._timeout, encoding="utf-8") as resp:
                raw  = resp.read()
...
class JovasFormatter:
    ...
    def formatFile(self, args):
        ...
        with open(path,"w", encoding="utf-8") as f: f.write(formatted)
...
class JovasDebugger:
    ...
    def exportLog(self, args=[]):
        ...
        with open(path,"w", encoding="utf-8") as f: json.dump(self._trace_log,f,indent=2,default=str)
